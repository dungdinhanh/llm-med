...
def load_instruct_data(file_path):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            instruct_data.append(image_data)
...
def save_results(matched_data, output_urls):
...
def main():
    ...
    parser.add_argument("--input_instruct", type=str, help="Path to the input instruction JSON file.", default="data/instruct/llava_med_instruct_60k_inline_mention.jsonl")
    ...
    parser.add_argument("--output_urls", type=str, help="Path to the output matched image URLs JSONL file.", default="data/matched_urls.jsonl_eval")
    
    args = parser.parse_args()
    
    instruct_data = load_instruct_data(args.input_instruct)
    image_url_dict = load_image_url_data(args.input_urls)
    matched_data = match_data(instruct_data, image_url_dict)
    save_results(matched_data, args.output_urls)
